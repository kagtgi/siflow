...
\IfFileExists{tables_auto.tex}{\input{tables_auto.tex}}{%
...
\paragraph{Setup.}
This section describes the evaluation protocol. The setup, baselines, metrics, and ablations below are fixed, and the numeric cells in Tables~\ref{tab:main}--\ref{tab:ablation} are auto-filled from the runs described in Table~\ref{tab:compute} (Appendix~\ref{app:impl}). The protocol is deliberately scoped to a \emph{single} A100-80GB GPU so that every reported number is reproducible without a cluster.

% CHANGE 11: Models paragraph — primary teacher is MDLM; Dream-7B added as -D; DiffusionGemma kept as -G; SEDD-small demoted to ablation
\paragraph{Models.}
{\sloppy
...
\textbf{\method{}-D: Dream-7B teacher.} Dream-7B~\citep{ye2025dream} is a 7B-parameter open masked DLM initialized from Qwen2.5-7B ($\sim$14\,GB fp16), covering a qualitatively different training regime---AR initialization, instruction tuning, and a superior quality-speed tradeoff at 5--20 steps. The student shares the Dream-7B backbone and tokenizer, training only the velocity head; teacher targets are cached offline at $\le\!64$ steps. \method{}-D validates cross-architecture generalization: the simplex-secant construction is agnostic to the teacher's internals.

\textbf{\method{}-G: DiffusionGemma teacher.} DiffusionGemma-26B-A4B~\citep{diffusiongemma2026} is a publicly released discrete diffusion LM (26B total, 3.8B active via sparse MoE; fp16 inference $\approx$7.6\,GB active weights). The student is initialized from the DiffusionGemma backbone, on its native tokenizer, and adds only the velocity head. Teacher targets are cached offline. We designate this variant \method{}-G; it validates that \method{} generalizes to large, MoE-based teachers and produces a one-step generator at billion-parameter scale.
...
\caption{Unconditional generation on OWT/LM1B.
  ...
  \method{}-D uses Dream-7B as teacher; \method{}-G uses DiffusionGemma-26B-A4B.
  $^\dagger$: number reported by the cited paper under its own protocol; all other rows
  measured by us on a single GPU. Tok/s and step counts include the velocity-head pass.}
\label{tab:main}
\small
...
\SiFlowMainRows
\midrule
\multicolumn{6}{l}{\itshape Head-only distillation on larger backbones (each on its native tokenizer):} \\
Dream-7B teacher~\citep{ye2025dream}   & $\sim$50 & -- & -- & -- & -- \\
\textbf{\method{}-D (ours)}           & 1      & -- & -- & -- & -- \\
\textbf{\method{}-D (ours)}           & 4      & -- & -- & -- & -- \\
\midrule
\multicolumn{6}{l}{\itshape DiffusionGemma backbone (head-only; A100-80GB):} \\
DiffusionGemma~\citep{diffusiongemma2026} & $\sim$1000 & -- & -- & -- & -- \\
\textbf{\method{}-G (ours)}           & 1      & -- & -- & -- & -- \\
\textbf{\method{}-G (ours)}           & 4      & -- & -- & -- & -- \\
\midrule
\multicolumn{6}{l}{\itshape Scale reference (different evaluation protocol):} \\
LLaDA-8B~\citep{nie2025llada}         & $\sim$256 & -- & -- & -- & -- \\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:compute} fixes the budget. The student is $\sim$170M, trained in bf16 with AdamW at learning rate $1\!\times\!10^{-4}$ on sequences of length $256$ (primary) up to $1024$ (secondary). Teacher targets are cached for a $\le\!2\!\times\!10^{5}$-sequence subset with reverse rollouts capped at $\le\!64$ steps. The full primary study fits in under $12$ hours on a single A100-80GB.

\begin{table}[t]
\centering
\caption{Single-GPU compute budget. Primary targets ${<}12$\,h on A100-80GB.}
\label{tab:compute}
\scriptsize
\begin{tabular}{@{}llr@{}}
\toprule
Phase & Config & Time \\
\midrule
Cache targets & ${\le}200$k seqs, ${\le}64$ steps & 2--3\,h \\
\method{} distil & 30--60k steps, bs\,32 & 6--8\,h \\
Eval & 5k samples, all baselines & 1--2\,h \\
Ablations & shared cache, head retrain & 2--3\,h \\
\midrule
\multicolumn{2}{l}{\textbf{Total primary (single A100-80\,GB)}} & ${<}12$\,h \\
\midrule
\multicolumn{3}{l}{\itshape \method{}-D (Dream-7B teacher)} \\
Cache -D & Dream-7B fp16, bs\,8 & 3--5\,h \\
Distil -D & head-only, same protocol & 6--8\,h \\
\multicolumn{2}{l}{\textbf{Total \method{}-D (A100-80\,GB)}} & ${<}14$\,h \\
\midrule
\multicolumn{3}{l}{\itshape \method{}-G (DiffusionGemma teacher)} \\
Cache -G & 3.8B active, fp16, bs\,4 & 3--5\,h \\
Distil -G & head-only, same protocol & 6--8\,h \\
\multicolumn{2}{l}{\textbf{Total \method{}-G (A100-80\,GB)}} & ${<}14$\,h \\
\bottomrule
\end{tabular}
\end{table}
...
(Dream, DiffusionGemma) we regress on a reduced top-$m$ support of the teacher predictive and fold
the remaining mass into a single ``rest'' bucket, which keeps the head and the cached targets compact.
...
$0.999$, and all reported numbers are averaged over seeds $\{0,1,2\}$.
